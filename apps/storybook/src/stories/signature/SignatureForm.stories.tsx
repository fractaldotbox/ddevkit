...
import { useConfig, usePublicClient, useSignTypedData, useVerifyTypedData, useWalletClient } from 'wagmi'
...
import { Account, createPublicClient, createWalletClient, http, TypedData, verifyMessage, verifyTypedData } from 'viem';
...
const useSign = (
    ...
    const { messageToVerify, typedData } = useMemo(() => {
        ...
        const messageToVerify = (signType === SignType.EIP191) ? message : {
            ...TYPED_DATA.message,
            contents: message
        }
...
const SignatureFormWagmi = ({
    ...
    const { signMessage, verifySignature, messageToVerify, signature, isReady } = useSign({
        account,
        signType,
        messageAtom,
        signatureAtom
...

                {
                    verifySignature && account?.address && (
                        <SignatureVerifyBadge
                            ...
                            message={messageToVerify}
                            signature={signature}
                            verify={async () => await verifySignature({
                                address: account.address,
                                message: messageToVerify,
                            })}
