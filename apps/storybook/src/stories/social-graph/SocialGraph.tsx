...
export const useFollowerAvatars = ({ addressOrEns }: { addressOrEns: AddressOrEns }) => {
...
export const SocialGraph = ({ addressOrEns }: { addressOrEns: AddressOrEns }) => {
    // TODO
    ...
    useEffect(() => {
        ...
        const followerAvatars = followers.slice(0, 20).map(
            ({
                address
            }) => {
                return {
        ...
        const gData = {
            nodes: followerAvatars.map(({ avatarUrl, id }) => ({ id, avatarUrl })).concat({
                id: addressOrEns as Address,
                avatarUrl: `https://euc.li/vitalik.eth`
            }),
            links: followerAvatars
                .map(({ id }) => ({
                    source: id,
                    target: addressOrEns
                }))
