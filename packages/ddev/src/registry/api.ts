...
export async function getRegistryIndex() {
	try {
		const [result] = await fetchRegistry(["http://ui.shadcn.com/r/index.json"]);
...
export async function getRegistryIcons() {
	try {
		const [result] = await fetchRegistry([
			"http://ui.shadcn.com/r/icons/index.json",
		]);
...
export async function getRegistryItem(name: string, style: string) {
	try {
		const [result] = await fetchRegistry([isUrl(name) ? name : `${name}.json`]);
